package dabba.doo.annotationprocessor.core.writer.spring.config;
...
import dabba.doo.annotationprocessor.db.executer.JdbcTemplateHolder;
...
public class SpringRestApiConfigWriter {
    ...
    private MethodSpec methodSpecForBeanConfigForNamedJdbcTemplate() {
        ...
        return MethodSpec.methodBuilder(getBeanName(classNameForJdbcTemplate.simpleName()))
                ...
                .returns(NamedParameterJdbcTemplate.class).build();
    }
    ...
    public JavaFile writeBeansConfigForApi(
        ...
        final Collection<MethodSpec> beanMethods = IntStream.range(0, layers.size()).mapToObj(
                index -> {
                    ...
                    return MethodSpec.methodBuilder(beanName)
                            ...
                            .returns(layers.get(index)).build();
                }
        ).collect(Collectors.toList());
        // beanMethods.add(methodSpecForBeanConfigForNamedJdbcTemplate());

        final TypeSpec.Builder typeSpecBuilder = TypeSpec.classBuilder(CONFIG_CLASS_NAME)
